...
Telegram 媒体下载机器人：把群里的**视频 / 图片相册 / 漫画 / 电子书 / 文件****转发给 bot**，交互式确认（推送目标 + 文件名）后自动下载，并推送到本机目录或远端服务器（rsync，支持密码/密钥认证）。

基于 Go + [gotgproto](https://github.com/celestix/gotgproto)（gotd/td MTProto），**无 20MB 下载限制**，支持 HTTP/SOCKS5 代理。

> 媒体类型自动识别：cbz/cbr/cb7 等 → 📖漫画；epub/mobi/azw3/fb2/txt 等 → 📚电子书；推送到哪个目录由你选目标决定。

## 使用流程

```
① 转发视频/图片相册/漫画/电子书/文件（相册自动聚合）给 bot
...
- **全部下载 / 多文件相册** → 自动建文件夹（文件夹名=识别名），整体推送（目标端保留文件夹名，如 `漫画/洞洞杂货店/第100话/`）
- **任何步骤可点 [❌取消]**，下载中取消立即中断
...
- 图片消息支持单图/相册（≤10 张）；漫画/电子书以文件（cbz/epub/mobi/txt…）或图片文档发送时均按原文件下载
- 超大漫画合集（数百张连发消息）请分批转发或打包成 cbz 文件发送
